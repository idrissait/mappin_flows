...
dest.xy <- subset(destination.xy, select = c(LATITUDE.x, LONGITUDE.x, LATITUDE.y, LONGITUDE.y, Member.type, Duration))

names(dest.xy)<- c("oX", "oY", "dX", "dY", "type", "Duration")


dest.xy <- ddply(dest.xy,~oX + oY + dX + dY + type,summarise,Duration=sum(Duration))
